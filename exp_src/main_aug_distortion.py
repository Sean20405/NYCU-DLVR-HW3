...
# Configuration
CONFIG = {
    'MODEL_NAME': 'maskrcnn_res50_aug_distortion',
...
class MaskRCNNSegmentation:
    ...
    def train(self):
        ...
        for epoch in range(self.config['NUM_EPOCHS']):
            ...
            if eval_metrics["mAP"] > best_map:
                ...
                self.save_model(f'{self.config["MODEL_NAME"]}_best.pth')
                print(f"New best model saved with mAP: {best_map:.4f}")

            # Save best model based on AP50
            if eval_metrics["AP50"] > best_ap50:
                best_ap50 = eval_metrics["AP50"]
                self.save_model(f'{self.config["MODEL_NAME"]}_best_ap50.pth')
    ...
    def load_model(self, filename):
        ...
        model_path = os.path.join(self.config['CKPT_PATH'], filename)
