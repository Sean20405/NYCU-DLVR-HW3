...
# Configuration
CONFIG = {
    'MODEL_NAME': 'maskrcnn_res50_partial_distortion',
...
class MaskRCNNSegmentation:
    ...
    def _get_model(self):
        ...
        # Unfreeze the last few layers (e.g., layer4)
        for parameter in model.backbone.body.layer4.parameters():
            parameter.requires_grad = True

        # Optionally unfreeze layer3 as well
        # for parameter in model.backbone.body.layer3.parameters():
        #     parameter.requires_grad = True

        # Print trainable parameters for verification
        print("Trainable parameters after partial freezing:")
        for name, parameter in model.named_parameters():
            if parameter.requires_grad:
                print(f"  - {name}")

        # Change the output size of the classifier
